...
function doGet(e) {
  ...
  try {
    ...
    } else if (action === 'notificar_revisor') {
      var dados = JSON.parse(e.parameter.data || '{}');
      result = notificarRevisor(dados);
    } else {
...
'<li>Crie sua senha pessoal (mínimo 6 caracteres)</li>' +
...
//  Helpers _mSec / _mPasso montam as seções para manter o HTML enxuto.
// ════════════════════════════════════════════════════════════════
...
function enviarManualUso(dados) {
  ...
  corpo += _mSec('🔑', 'Entrar no sistema', 'Login e primeiro acesso', '' +
    _mItem('Primeiro acesso', _mPasso([
      ...
      'Aparece "👋 Primeiro acesso". <strong>Crie sua senha</strong> (mín. 6 caracteres), confirme e clique em <strong>Entrar</strong>'
    ])) +
...
// ════════════════════════════════════════════════════════════════
//  NOTIFICAR REVISOR (documento enviado para revisao no juridico)
//  Payload esperado:
//    { email, revisor, autor, protocolo, tipo, referencia, mensagem, link }
// ════════════════════════════════════════════════════════════════
function notificarRevisor(dados) {
  var email      = dados.email;
  var revisor    = dados.revisor    || 'colega';
  var autor      = dados.autor      || 'um colega';
  var protocolo  = dados.protocolo  || '';
  var tipo       = dados.tipo       || 'documento';
  var referencia = dados.referencia || '';
  var mensagem   = dados.mensagem   || '';
  var link       = dados.link       || 'https://falcaovaz.netlify.app';

  if (!email) return { success: false, error: 'email obrigatorio' };

  var primeiroNome = String(revisor).split(' ')[0];

  var html = '<!DOCTYPE html><html><body style="margin:0;padding:0;background:#f1f5f9;font-family:-apple-system,Segoe UI,Arial,sans-serif">' +
    ...
    '<p style="color:#fff;font-size:18px;font-weight:700;margin:0">⚖️ Documento aguardando sua revisão</p>' +
    '</div>' +

    '<div style="padding:28px 32px">' +
    '<p style="font-size:14px;color:#334155;line-height:1.7">Olá, <strong>' + primeiroNome + '</strong>!</p>' +
    '<p style="font-size:14px;color:#334155;line-height:1.7"><strong>' + autor + '</strong> enviou um documento pra você revisar no sistema jurídico:</p>' +

    '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:16px 20px;margin:16px 0">' +
    (protocolo  ? '<p style="margin:0 0 6px;font-size:13px;color:#64748b"><strong style="color:#1e293b">Protocolo:</strong> ' + protocolo + '</p>' : '') +
    '<p style="margin:0 0 6px;font-size:13px;color:#64748b"><strong style="color:#1e293b">Tipo:</strong> ' + tipo + '</p>' +
    (referencia ? '<p style="margin:0 0 6px;font-size:13px;color:#64748b"><strong style="color:#1e293b">Referência:</strong> ' + referencia + '</p>' : '') +
    (mensagem   ? '<p style="margin:6px 0 0;font-size:13px;color:#475569;border-top:1px solid #e2e8f0;padding-top:10px"><em>“' + mensagem + '”</em></p>' : '') +
    '</div>' +

    '<div style="text-align:center;margin:24px 0 8px">' +
    '<a href="' + link + '" style="background:#2563eb;color:#fff;text-decoration:none;font-size:14px;font-weight:700;padding:12px 28px;border-radius:8px;display:inline-block">Abrir o sistema e revisar</a>' +
    '</div>' +
    '<p style="font-size:12px;color:#94a3b8;text-align:center">O documento está na aba <strong>Para Revisar</strong>.</p>' +
    '</div>' +

    '<div style="background:#f8fafc;padding:20px 32px;border-top:1px solid #e2e8f0;text-align:center">' +
    '<p style="font-size:12px;color:#94a3b8;margin:0">Nova São Paulo Imobiliária · Sistema Jurídico</p>' +
    '<p style="font-size:11px;color:#cbd5e1;margin:4px 0 0">Este email foi enviado automaticamente pelo sistema jurídico.</p>' +
    '</div>' +
  ...
  MailApp.sendEmail({
    to:       email,
    subject:  '⚖️ Documento para revisão' + (protocolo ? ' — ' + protocolo : '') + ' — ' + autor,
    htmlBody: html,
    name:     EMAIL_REMETENTE,
    replyTo:  REPLY_TO
  });
